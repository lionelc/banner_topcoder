...
public class CRFTagger implements Tagger
{
	...
	public static CRFTagger train(Set<Sentence> sentences, int order, TagFormat format, FeatureSet featureSet)
	{
		...
		// TODO Make this configurable
		 crfTrainer.train(instances, 30, new double[] {0.2});  
		 
		 crfTrainer.train(instances, 50, new double[] {0.4});
		 
		 crfTrainer.train(instances, 80, new double[] {0.6});
		
		 crfTrainer.train(instances, 120, new double[] {0.8});
		
		//crfTrainer.train(instances, 10, new double[] { 0.2, 0.5, 0.8 });  //LJ 03/15
		// Train to convergence
		crfTrainer.train(instances, 500);  //220
		//crfTrainer.train(instances);  //LJ 03/15
		return new CRFTagger(model, featureSet, order);
	}
